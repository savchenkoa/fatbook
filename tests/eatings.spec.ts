...
test.describe.serial("Eatings Flow", () => {
    test("should add eating, modify weight, and delete eating", async ({ page }) => {
        ...
        await expect(page.getByText("Breakfast").locator("..").locator("..")).toBeVisible();
        ...
        await expect(page).toHaveURL(/\/eatings\/\d{2}-\w{3}-\d{4}\/breakfast\/add/);
        ...
        async function addEatingToMeal(
            ...
            if (!found) {
                ...
                const firstDish = page.locator("div").filter({ hasText: /kcal/ }).first();
