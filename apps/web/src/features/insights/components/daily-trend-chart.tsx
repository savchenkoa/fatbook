...
function CustomTooltip({ active, payload, yKey, referenceUnits }: CustomTooltipProps) {
...
export function DailyTrendChart({
    ...
    const transformedData = data?.map((item) => ({
        ...
        excessValue: item[yKey] > (referenceValue || Infinity) ? item[yKey] - referenceValue! : 0,
    }));
        ...
        <Box className="mx-4 mb-4 p-4 sm:mx-0 sm:p-6">
            <div className="items-baseine mb-2 flex justify-between">
                ...
                <span className="flex items-center gap-1 text-xs text-gray-500">
                    <LucideGoal className="size-4" /> {referenceValue} {referenceUnits} / day
                </span>
            </div>
